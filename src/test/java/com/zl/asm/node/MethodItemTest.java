...
class MethodItemTest {

    byte[] methodItemBytes = new byte[]{0, 1, 0, 12, 0, 13, 0, 1, 0, 14, 0, 0, 0, 57, 0, 2, 0, 1, 0, 0, 0, 11, 42, -73, 0, 1,
            42, 16, 10, -75, 0, 2, -79, 0, 0, 0, 2, 0, 15, 0, 0, 0, 10, 0, 2, 0, 0, 0, 3, 0, 4, 0, 6, 0, 16, 0, 0, 0,
            12, 0, 1, 0, 0, 0, 11, 0, 17, 0, 18, 0, 0};
    ...
    public void test(){
        ...
        MethodItem methodItem = new MethodItem(methodItemByteContainer, node);
        AccessFlag accessFlags = methodItem.getAccessFlags();
        List<String> accessFlags1 = accessFlags.getAccessFlags();
        assertArrayEquals(new String[]{"ACC_PUBLIC"}, accessFlags1.toArray());
        int nameIndex = methodItem.getNameIndex();
        assertEquals(12, nameIndex);
        int descriptorIndex = methodItem.getDescriptorIndex();
        assertEquals(13, descriptorIndex);
        int attributesCount = methodItem.getAttributesCount();
        assertEquals(1, attributesCount);
        Attribute[] attributes = methodItem.getAttributes();
